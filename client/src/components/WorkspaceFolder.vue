    ...
    <div class="folder-label" :style="indent" :class="wsPath === systemConfig.root + info.path ? 'selected' : ''">
        ...
        <span class="icon-wrapper expand-icon">
          ...
          <template v-else>
            <template v-if="info.hasSubFolders">
              <b-icon v-if="showChildren" icon="triangle-fill" class="option-icon clickable-icon"
                @click="toggleShowChildren" />
              <b-icon v-else rotate="90" icon="triangle-fill" class="option-icon clickable-icon"
                @click="toggleShowChildren" />
            </template>
            <span v-else class="margin-keeper" />
          </template>
        </span>
        <span class="name" @click="setWorkspace" :style="{
      ...
      <div class="options">
        <div class="option-progress" title="Percentage Classified">
          ...
          <span class="option-progress-text" v-if="
            showSubFolderProgress &&
            info.name.toLowerCase() !== 'unclassified' &&
            totalFiles > 0
          ">
            {{ progress }}%</span>
          <progress :max="100" :value="progress" class="ws-progress" v-bind:class="{
            'opacity-0':
        ...
        <div>
          <div class="icon-wrapper" title="AI Statistic">
            <b-icon icon="bar-chart-fill" :class="canViewStatistics ? 'clickable-icon' : 'gray-icon'" font-scale="1.5"
              @click="showStatistic" />
          </div>
          <div class="icon-wrapper" title="Notes">
            <b-icon :icon="info && info.highlight ? 'file-text-fill' : 'file-text'" class="svg-icon clickable-icon"
              :class="info.highlight ? 'highlight' : ''" @click="showNotes" />
          </div>
          <div v-if="canSeeConfusionMatrix" class="icon-wrapper clickable" @click="showConfusionMatrix()"
            title="Compare Workspaces">
            <svg xmlns="http://www.w3.org/2000/svg" width="20px" height="20px" viewBox="0 0 100 103"
              preserveAspectRatio="xMidYMid meet">
              <g transform="translate(0.000000,103.000000) scale(0.100000,-0.100000)" :fill="
          ...
          <div v-if="canSyncDB" class="icon-wrapper" title="Convert to Database">
            <b-icon :animation="DBSyncing ? 'fade' : undefined" :class="info.isDB ? 'gray-icon' : 'clickable-icon'"
              icon="shuffle" font-scale="1.5" @click="syncDB" />
          </div>
          <div class="icon-wrapper" title="Workspace Settings">
            <b-icon :class="
              info.config && canEditConfig ? 'clickable-icon' : 'gray-icon'
            " icon="gear-fill" font-scale="1.5" @click="showConfig" />
          </div>
          <div v-if="canBackup" class="icon-wrapper" title="Backup Workspace">
            <b-icon class="clickable-icon" icon="server" font-scale="1.5" @click="backup()" />
          </div>
        </div>
      </div>
    </div>
    <div v-if="hasChildren && showChildren">
      <workspace-folder v-for="(folder, index) in info.subFolders" :rawInfo="folder" :depth="depth + 1"
        :key-path="`${keyPath}.subFolders.[${index}]`" :key="folder.path"
        :show-sub-folder-progress="showSubFolderProgress" @select-workspace="$emit('select-workspace', $event)" />
      <div class="new-ws" :style="{
        marginLeft: `${(depth + 1) * 50}px`,
      }" v-if="systemConfig.newWorkspace && newWorkspace" @click="createNewFolder">
