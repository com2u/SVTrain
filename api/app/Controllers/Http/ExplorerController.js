        ...
        subfolders.map(async anotherSubfolder => {
          ...
          await Promise.all(
            flist.map(async fileFromSF => {
              ...
              if (fileFromSF_lstat.isFile() && fileFromSF.replace(/___.*/, '').toLowerCase().includes(subfolder.toLowerCase())) {
  ...
  await Promise.all(
    files.map(async (f) => {
      ...
      if (
        ...
        if (f.replace(/___.*/, '').toLowerCase().indexOf(dirname.toLowerCase()) > -1) {
        ...
        if (f)
          if (flstat.isDirectory()) {
            ...
            result.files.push({
              ...
              match: f.replace(/___.*/, '').toLowerCase().indexOf(dir.split(path.sep)[dir.split(path.sep).length - 1].toLowerCase()) > -1,
              size: flstat.size,
